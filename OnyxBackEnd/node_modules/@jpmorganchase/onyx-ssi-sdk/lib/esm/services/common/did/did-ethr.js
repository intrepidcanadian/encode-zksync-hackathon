...
export class EthrDIDMethod {
    ...
    async generateFromPrivateKey(privateKey) {
        ...
        const publicKey = KeyUtils.privateKeyToPublicKey(privateKey);
        const address = new Wallet(privateKey, this.web3Provider).address;
     ...
     */
    async update(did, newPublicKey) {
        if (!KeyUtils.isHexPublicKey(newPublicKey, true)) {
            throw new DIDMethodFailureError('new public key not in hex format');
        }
        const wallet = new Wallet(did.keyPair.privateKey, this.web3Provider);
        ...
        const tx = await registry.setAttribute(address, ethers.utils.formatBytes32String('did/pub/Secp256k1/veriKey/hex'), newPublicKey, 10000000000000);
    ...
    async deactivate(did) {
        ...
        const registry = new Contract(contractAddress, EthereumDIDRegistry.abi, wallet);
        const tx = await registry.changeOwner(address, '0x0000000000000000000000000000000000000000');
        const receipt = await tx.wait();
        if (!receipt || !receipt.status) {
