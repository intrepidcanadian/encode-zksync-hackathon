...
import { SchemaManager } from '../common';
 ...
 */
export async function verifyCredentialJWT(vc, didResolver, options
 ...
 * `TypeError` if the input is not W3C compliant
 */
export async function verifyPresentationJWT(vp, didResolver, options
// eslint-disable-next-line  @typescript-eslint/no-explicit-any
) {
    if (typeof vp === 'string') {
 ...
 */
export async function verifyDID(did, didResolver) {
 ...
 */
export async function verifyDIDs(vc, didResolver) {
    let issuer;
    let holder;
    if (typeof vc === 'string') {
 ...
 */
export function verifyExpiry(vc) {
    let expiryDate;
    if (typeof vc === 'string') {
        const credential = decodeJWT(vc).payload;
        if (!credential.exp) {
 ...
 */
export function verifyIssuanceDate(vc) {
    let issuanceDate;
    if (typeof vc === 'string') {
        const credential = decodeJWT(vc).payload;
        if (!credential.nbf) {
 ...
 */
export async function verifyRevocationStatus(vc, didResolver) {
    let vcId;
    if (typeof vc === 'string') {
        const credential = decodeJWT(vc).payload;
 ...
 */
export async function verifySchema(vc, isFile) {
    let credSchema;
    let credSubject;
    if (typeof vc === 'string') {
        const credential = decodeJWT(vc).payload;
