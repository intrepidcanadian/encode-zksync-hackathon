...
class EthrDIDMethod {
    constructor(providerConfigs) {
        ...
        this.web3Provider = providerConfigs.provider ? providerConfigs.provider : new providers_1.JsonRpcProvider(providerConfigs.rpcUrl);
    }
    /**
     *
    ...
    create() {
        return __awaiter(this, void 0, void 0, function* () {
            const account = yield ethers_1.ethers.Wallet.createRandom();
            const privateKey = account.privateKey;
            const publicKey = utils_1.KeyUtils.privateKeyToPublicKey(privateKey);
            const did = `did:ethr:${this.providerConfigs.name}:${account.address}`;
            const identity = {
                did,
                keyPair: {
                    algorithm: utils_1.KEY_ALG.ES256K,
                    publicKey,
                    privateKey
                }
            };
    ...
    generateFromPrivateKey(privateKey) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!utils_1.KeyUtils.isHexPrivateKey(privateKey)) {
                throw new errors_1.DIDMethodFailureError('new public key not in hex format');
            }
            const publicKey = utils_1.KeyUtils.privateKeyToPublicKey(privateKey);
            const address = new wallet_1.Wallet(privateKey, this.web3Provider).address;
    ...
    resolve(did) {
        return __awaiter(this, void 0, void 0, function* () {
            const ethrDidResolver = new did_resolver_1.Resolver((0, ethr_did_resolver_1.getResolver)(this.providerConfigs));
            const result = yield ethrDidResolver.resolve(did);
     ...
     */
    update(did, newPublicKey) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!utils_1.KeyUtils.isHexPublicKey(newPublicKey, true)) {
                throw new errors_1.DIDMethodFailureError('new public key not in hex format');
            }
            const wallet = new wallet_1.Wallet(did.keyPair.privateKey, this.web3Provider);
            const address = yield wallet.connect(this.web3Provider).getAddress();
            const contractAddress = this.providerConfigs.registry;
            const registry = new contracts_1.Contract(contractAddress, ethr_did_resolver_1.EthereumDIDRegistry.abi, wallet);
            const tx = yield registry.setAttribute(address, ethers_1.ethers.utils.formatBytes32String('did/pub/Secp256k1/veriKey/hex'), newPublicKey, 10000000000000);
            const receipt = yield tx.wait();
            if (!receipt || !receipt.status) {
                throw new errors_1.DIDMethodFailureError('Error updating');
            }
    ...
    deactivate(did) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const registry = new contracts_1.Contract(contractAddress, ethr_did_resolver_1.EthereumDIDRegistry.abi, wallet);
            const tx = yield registry.changeOwner(address, '0x0000000000000000000000000000000000000000');
            const receipt = yield tx.wait();
            if (!receipt || !receipt.status) {
                throw new errors_1.DIDMethodFailureError('Error deactivating');
            }
            return Promise.resolve(true);
        });
    }
    ...
    isActive(did) {
        return __awaiter(this, void 0, void 0, function* () {
            const didResult = yield this.resolve(did);
    ...
    convertDIDToAddress(did) {
        ...
        switch (format.length) {
            ...
            default: {
                throw new errors_1.DIDMethodFailureError(`did:ethr ${did} not in correct format`);
            }
        }
        try {
            ...
            throw new errors_1.DIDMethodFailureError(`Cannot convert identifier in ${did} to an Ethereum address`);
        }
        return address;
    }
    /**
     ...
     */
    getDIDResolver() {
        return (0, ethr_did_resolver_1.getResolver)(this.providerConfigs);
    }
}
exports.EthrDIDMethod = EthrDIDMethod;
